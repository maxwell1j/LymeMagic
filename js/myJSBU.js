...
myTest = function () {
console.log(getSessionId ().toString());
    
logLocation  (getSessionId (),lat,lng,sessionStorage.scorePoints,3,sessionStorage.userName) ;
 }
 ...
 function audioInitiate()
{
   ...
   if (audioInitaited == false) {
    ...
    audioInitaited = true;
    //Juery to loop through all audio files
}
...
var accuracyThreshold = 10; //accucy in meters that count as accurate.  
    ...
    if (lat == undefined){inital = true}else{inital = false} ;//Check if first location find of user
    ...
    if(distance > 0 && distance < 0.003){distanceScore = Math.round(distance*1000);}
     ...
     if  ( position.coords.accuracy > accuracyThreshold && gpsAttempts < 3 )  
       ...
       if (distance > 0)
            {// only move if aps is accurate or number of attempts have been made
                console.log("gps OK") ;
           ...
           addPoints(distanceScore);
            ...
            gpsAttempts = 0
            moveMarker(markerWizard,latlngWizard);
...
function showMap(lat, lon) {
 ...
 var mapOptionsTrail = {
   ...
   markerWizard = new google.maps.Marker({
    ...
    secondSprite.marker.icon = "img/easterBunnyIcon.png";
    secondSprite.soundCollision = "audiomagicEggs";
    secondSprite.iconEnd = "img/eggIcon.png";
 ...
 //Move all sprites in Circle   
   for (index = 0; index < sprites.length; index++)
    ...
    if(alerted == true && alertClose == false){
                                            ...
                                            document.getElementById('menuLinkMap').click(); //Show Map
                                            }
...
function geolocationError (){
    ...
    messageNotify("Haveing trouble with GPS");
     document.getElementById("scoreMap").innerHTML = "Having a bit of trouble with Magic scanner, so no score to give";
}   
     ...
     function addPoints (points)  {
         ...
         if (sessionStorage.scorePoints) {
            ...
            //Display Score: function for this at later stage
           document.getElementById("scoreMap").innerHTML = "You have " +
        Number(sessionStorage.scorePoints).toLocaleString() + " magical points."; 
